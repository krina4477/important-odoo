...
class SaleOrder(models.Model):
    ...
    @api.constrains('sale_manual_currency_rate_active', 'sale_manual_currency_rate', 'currency_id')
    def _check_manual_currency_validation(self):
        for rec in self:
            if rec.sale_manual_currency_rate_active:
                if rec.sale_manual_currency_rate <= 0.0:
