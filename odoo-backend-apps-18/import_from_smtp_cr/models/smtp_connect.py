...
class SmtpImportConfiguration(models.Model):
    _name = 'smtp.import.config'
    _rec_name = 'smtp_host'

    smtp_host = fields.Many2one('smtp.connect', required=True)
    ...
    source_destination_paths = fields.One2many('source.destination.paths', 'smtp_config_id', string="Source Destination Configuration")
    model = fields.Many2one('ir.model')
    fields = fields.One2many('smtp.import.config.fields', 'import_config_id', string='Fields')
    ...
    def _read_from_smtp_and_import(self):
        import_config_ids = self.search([])
        for import_config in import_config_ids:
            import_config.smtp_host.test_connection()

            hostname = import_config.smtp_host.host_name
            username = import_config.smtp_host.username
            password = import_config.smtp_host.password
            ...
            if import_config.smtp_host.with_password:
                ssh.connect(hostname,username, password=password)
            else:
                ssh.connect(hostname, username=username, password=password, key_filename=import_config.smtp_host.public_key_path)
            ...
            for path in import_config.source_destination_paths:
                ...
                try:
                    ...
                    files = smtp.listdir(path.remote_source_dirpath)
                    import_list = []
                    for file in files:
                ...
                for f in import_list:
                    if ' ' not in f and (f.endswith('.csv') or f.endswith('.xls') or f.endswith('.xlsx')):
                        ...
                        with smtp.open(remote_file) as open_file:
                        ...
                        if date_str not in smtp.listdir(path.remote_archive_dirpath):
                            smtp.mkdir(new_dir)

                        # move files from one to another folder
                        smtp.posix_rename(remote_file, new_dir+'/'+f)
                    else:
                        raise MissingError('No file found for Import!')

                # close the SMTP session and the SSH connection
                self.env.cr.commit()
                smtp.close()
            ssh.close()


class SmtpImportConfigurationFields(models.Model):
    _name = 'smtp.import.config.fields'
    _rec_name = 'import_config_id'

    import_config_id = fields.Many2one('smtp.import.config', 'smtp Import Config')
...
class SmtpSourceDesctinationPaths(models.Model):
    _name = 'source.destination.paths'
    _rec_name = 'smtp_config_id'

    smtp_config_id = fields.Many2one('smtp.import.config', string="Smtp Import Config")
