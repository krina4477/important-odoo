...
class PosOrder(models.Model):
    ...
    pos_reference_barcode = fields.Char(string="Barcode Reference", compute="_compute_pos_reference_barcode", store=True)
    ...
    def _compute_pos_reference_barcode(self):
        for rec in self:
            if rec.pos_reference:
                cleaned_order = re.sub(r'Order|[\s-]', '', rec.pos_reference)
                rec.pos_reference_barcode = cleaned_order
        ...
        if config_data['sh_load_order_by']:
            ...
            if config_data['sh_load_order_by'] == 'session_wise':
                ...
                if config_data['sh_session_wise_option'] == 'current_session':
                    order_data = self.env['pos.order'].search_read(['|', ('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']), ('assigned_config', '=', config_data['id']), (
            ...
            if config_data['sh_load_order_by'] == 'all':
                order_data = self.env['pos.order'].search_read(['|', ('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']), (
            ...
            if config_data['sh_load_order_by'] == 'day_wise':
                ...
                if config_data['sh_day_wise_option'] == 'current_day':
                    ...
                    order_data = self.env['pos.order'].search_read(['|', ('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']), ('assigned_config', '=', config_data['id']), ('date_order', '>=', (
                ...
                if config_data['sh_session_wise_option'] == 'last_no_session':
                    ...
                    if session:
                        order_data = self.env['pos.order'].search_read(
                            [('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']), ('session_id', 'in', session), ('state', '!=', 'cancel')])

            if config_data['sh_load_order_by'] == 'all':
                order_data = self.env['pos.order'].search_read(
                    [('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']), ('state', '!=', 'cancel')])

            if config_data['sh_load_order_by'] == 'day_wise':

                if config_data['sh_day_wise_option'] == 'current_day':
                    today_date = datetime.today().strftime('%Y-%m-%d')
                    order_data = self.env['pos.order'].search_read([('employee_id', 'in', config_data['employee_ids']), ('session_id.config_id.name', '=', config_data['name']),('date_order', '>=', (
    ...
    def search_return_order(self, config_data, page_number):
        ...
        if config_data['sh_load_order_by']:
            ...
            if config_data['sh_load_order_by'] == 'session_wise':
                ...
                if config_data['sh_session_wise_option'] == 'current_session':
                    order_data = self.env['pos.order'].search_read([('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']), ('session_id', '=', config_data['current_session_id'][0]), (
                ...
                if config_data['sh_session_wise_option'] == 'last_no_session':
                    ...
                    for x in range(0, config_data['sh_last_no_session']):
                        if x < len(all_session):
                            session.append(all_session[x]['id'])
                    if session:
                        order_data = self.env['pos.order'].search_read([('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']), ('session_id', 'in', session), (
                            'state', '!=', 'cancel'), ('is_return_order', '=', False), ('is_exchange_order', '=', False)], limit=showTo)
            ...
            if config_data['sh_load_order_by'] == 'day_wise':
                ...
                if config_data['sh_day_wise_option'] == 'current_day':
                    today_date = datetime.today().strftime('%Y-%m-%d')
                    order_data = self.env['pos.order'].search_read([('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']),('date_order', '>=', (today_date + " 00:00:00")), (
                ...
                if config_data['sh_day_wise_option'] == 'last_no_day':
                    if config_data['sh_last_no_days']:
                        ...
                        last_date = datetime.today() - \
                            timedelta(days=config_data['sh_last_no_days'])
                        last_date = last_date.strftime('%Y-%m-%d')
                        order_data = self.env['pos.order'].search_read([('employee_id', 'in', config_data['employee_ids']),('session_id.config_id.name', '=', config_data['name']),('date_order', '<=', (today_date + " 24:00:00")), (
