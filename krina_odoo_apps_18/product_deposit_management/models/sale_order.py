...
class SaleOrder(models.Model):
    ...
    def _apply_order_line_sequence(self):
        ...
        old_line_data = [line.copy_data()[0] for line in
                         self.order_line.filtered(lambda sol: not sol.product_id.deposit_type)]

        for order_line in self.order_line.filtered(lambda sol: sol.product_id.deposit_type == 'has_deposit'):
    ...
    def _get_order_lines_to_report(self):
        ...
        sorted_order_lines = order_lines.sorted(lambda line: line.sequence, reverse=True)
