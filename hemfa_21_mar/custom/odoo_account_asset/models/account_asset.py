...
#    domain=[('internal_type','=','other'), ('deprecated', '=', False)],
    ...
    account_depreciation_id = fields.Many2one('account.account', string='Depreciation Entries: Asset Account', required=True, 
#    domain=[('internal_type','=','other'), ('deprecated', '=', False)],
    help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Depreciation Entries: Expense Account', required=True, 
#    domain=[('internal_type','=','other'), ('deprecated', '=', False)],
    oldname='account_income_recognition_id', help="Account used in the periodical entries, to record a part of the asset as expense.")
...
#                            days = (self.company_id.custom_compute_fiscalyear_dates(depreciation_date)['date_to'] - depreciation_date).days + 1
                            days = (self.company_id.custom_compute_fiscalyear_dates(depreciation_date)['date_to'] - depreciation_date).days + 1
...
#                'analytic_account_id': category_id.account_analytic_id.id if category_id.type == 'sale' else False,
            ...
            if category_id.type == 'sale' and category_id.account_analytic_id:
                move_line_1.update({
                    'analytic_distribution': {category_id.account_analytic_id.id : 100},
                })
            move_line_2 = {
                'name': asset_name,
...
#                'analytic_account_id': category_id.account_analytic_id.id if category_id.type == 'purchase' else False,
            ...
            if category_id.type == 'purchase' and category_id.account_analytic_id:
                move_line_2.update({
                    'analytic_distribution': {category_id.account_analytic_id.id : 100},
                })
            move_vals = {
        ...
        if post_move and created_moves:
            created_moves.filtered(lambda m: any(m.asset_depreciation_ids.mapped('asset_id.category_id.open_asset')))._post()
