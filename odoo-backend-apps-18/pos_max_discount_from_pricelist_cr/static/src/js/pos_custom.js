                ...
                if (posmodel.config.use_pricelist){
                    ...
                    element.forEach((pricelists) =>{
                        ...
                        var pricelist_items = items.filter((item) => {
                            return (item.raw.product_tmpl_id === self.get_selected_orderline().product_id.raw.product_tmpl_id) &&
                                   (item.raw.product_id ? item.raw.product_id === self.get_selected_orderline().product_id.raw.id : true) &&
                                   (item.raw.categ_id ? category_ids.contains(item.raw.categ_id[0]) : true) &&
                                   (item.raw.date_start ? moment(item.raw.date_start).isSameOrBefore(date) : true) &&
                                   (item.raw.date_end ? moment(item.raw.date_end).isSameOrAfter(date) : true);
                        });
                    ...
                    Object.keys(pricelist_price_dict).forEach((maxpricelist) =>{
                        ...
                        var pricelist = posmodel.data.models["product.pricelist"].find((pricelist) => pricelist.id === parseInt(minpriceKey));
                        line.set_pricelist_discount_label(policy);
                        line.set_pricelist_label(name);
                        line.set_pricelist_id(pricelist);
        ...
        if (posmodel.config.use_pricelist){
            ...
            posmodel.config.available_pricelist_ids.forEach(function (pricelists) {
                var pricelist_items = pricelists.item_ids.filter(function (item) {
                    return (item.raw.product_tmpl_id === self.raw.product_tmpl_id) &&
                        (item.raw.product_id ? item.raw.product_id === self.id : true) &&
                        (item.raw.categ_id ? category_ids.contains(item.raw.categ_id[0]) : true) &&
                        (item.raw.date_start ? moment(item._raw.date_start).isSameOrBefore(date) : true) &&
                        (item.raw.date_end ? moment(item._raw.date_end).isSameOrAfter(date) : true);
                });
                var pricelist_rule_items = {}
	    		...
	    		if(price > pricelist_rule_items[minKey]) {
	    			price = pricelist_rule_items[minKey]
	    		}
            });
