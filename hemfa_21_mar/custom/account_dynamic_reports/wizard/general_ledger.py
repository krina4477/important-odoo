...
class InsGeneralLedger(models.TransientModel):
    ...
    saleperson_ids = fields.Many2many(
        'res.users', string='SalePersons'
    )
    ...
    analytic_distribution_ids = fields.Many2many('account.analytic.account', 'general_line_analytic_distribution_rel')

    salepersons_ids = fields.Many2many('res.users', 'general_line_res_users_rel')

    currency_ids = fields.Many2many('res.currency', 'general_line_res_currency_rel')
    ...
    def write(self, vals):
        ...
        if vals.get('saleperson_ids'):
            vals.update({'saleperson_ids': vals.get('saleperson_ids')})

        if vals.get('analytic_ids') == []:
            vals.update({'analytic_ids': [(5,)]})

        if vals.get('saleperson_ids') == []:
            vals.update({'saleperson_ids': [(5,)]})
    ...
    def process_filters(self):
        ...
        if data.get('saleperson_ids', []):
            filters['saleperson_ids'] = self.env['res.partner'].browse(data.get('saleperson_ids', [])).mapped('name')
        else:
            filters['saleperson_ids'] = ['All']
    ...
    def build_where_clause(self, data=False):
        ...
        if data:
            ...
            if self.branch_ids:
                if data.get('branch_ids', []):
                    WHERE += ' AND b.id IN %s' % str(tuple(data.get('branch_ids')) + tuple([0]))
            if data.get('analytic_ids', []):
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        if self.branch_ids:
            sql = ('''
                ...
                LEFT JOIN move_line_analytic_distribution_rel line_analytic on (line_analytic.line_id = l.id)
                WHERE %s
                GROUP BY j.code, p.name, l.date, l.move_id, b.name
                ORDER BY %s
                OFFSET %s ROWS
    ...
    def get_filters(self, default_filters={}):
        ...
        branch_ids = self.branch_ids if self.branch_ids else self.env['res.branch'].search(company_domain)
        ...
        filter_dict = {
            ...
            'branch_list': [(j.id, j.name) for j in branch_ids],
            ...
            # 'branches': [(a.id, a.name) for a in self.env['res.branch'].search([])],
            # 'branch_ids': self.branch_ids.ids,
        }
    ...
    def action_xlsx(self):
        ...
        a_list = ', '.join([lt[1] or '' for lt in filter.get('branch_list')])
        sheet_2.write(row_pos_2, 1, a_list, content_header)
                        ...
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            if 'amount_currency' in sub_line and sub_line.get('currency_symbol') != sub_line.get(
                                ...
                                if sub_line.get('currency_position') == 'before':
                                    amount = sub_line.get('currency_symbol') + ' ' + str(
                                        float(sub_line.get('amount_currency')))
                                else:
                                    amount = str(float(sub_line.get('amount_currency'))) + ' ' + sub_line.get(
                                        'currency_symbol')
                            ...
                            if account_lines[line].get('balance_in_fc') and account_lines[line].get(
                                    'account_currency_id') != account_lines[line].get('company_currency_id'):
                                if account_lines[line].get('account_currency_id'):
                                    account_currency_id = account_currency_obj.browse(
                                        account_lines[line].get('account_currency_id'))
                                    if account_currency_id.position == 'before':
                                        amount = account_currency_id.symbol + ' ' + str(
                                            float(account_lines[line].get('balance_in_fc')))
                                    else:
                                        amount = str(float(account_lines[line].get(
                                            'balance_in_fc'))) + ' ' + account_currency_id.symbol
                                else:
