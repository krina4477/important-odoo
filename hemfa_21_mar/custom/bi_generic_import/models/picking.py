...
#
	...
	def create_picking(self, values):
		picking_search = self.env['stock.picking']
		if values.get('name'):
			picking_search = self.env['stock.picking'].search([
		...
		if picking_search:
			...
			if not self.is_two_step_transfer and not self.transit_location_id:
			...
			for i in range(len(reader_info)):
				...
				if values:
					if i == 0:
						...
						if not values.get('name') and picking_name:
							values.update({'name': picking_name})
						res = self.create_picking(values)
						if not values.get('name') and res:
			...
			for row_no in range(sheet.nrows):
				...
				if row_no <= 0:
					...
					if line[3]:
						...
						date_string = False
					values.update( {
									'name': line[0] if line[0] else picking_name,
									'customer': line[1],
									'origin':line[2],
									'product': line[4],
									'quantity': line[5],
