...
class InsPartnerLedger(models.TransientModel):
    ...
    display_accounts = fields.Selection(
        [('all', 'All'),
         ('balance_not_zero', 'With balance not equal to zero'), ('balance_is_zero', 'With balance is zero')],
        string='Display accounts',
    )

    @api.constrains('display_accounts')
    def _constrains_display_accounts(self):
        for rec in self:
            if rec.display_accounts == 'balance_is_zero':
                rec.balance_is_zero = True
    ...
    balance_is_zero = fields.Boolean(string='With balance is zero')
    ...
    salepersons_ids = fields.Many2many('res.users', 'partner_line_res_users_rel')

    currency_ids = fields.Many2many('res.currency', 'partner_line_res_currency_rel')
    ...
    def write(self, vals):
        ...
        if vals.get('branch_ids'):
            vals.update({'branch_ids': vals.get('branch_ids')})
        if vals.get('journal_ids') == []:
        ...
        if vals.get('analytic_distribution_ids') == []:
            vals.update({'analytic_distribution_ids': [(5,)]})

        if vals.get('branch_ids') == []:
            vals.update({'branch_ids': [(5,)]})

        if vals.get('salepersons_ids') == []:
            vals.update({'salepersons_ids': [(5,)]})
    ...
    def process_filters(self):
        ...
        if data.get('branch_ids', []):
            filters['branches'] = self.env['res.branch'].browse(data.get('branch_ids', [])).mapped('name')
        else:
        ...
        if data.get('currency_ids', []):
            filters['currency_ids'] = self.env['res.currency'].browse(data.get('currency_ids', [])).mapped('name')
        else:
            filters['currency_ids'] = ['All']

        filters['journals_list'] = data.get('journals_list')
        filters['branch_list'] = data.get('branch_list')
        ...
        filters['analytic_distribution_list'] = data.get('analytic_distribution_list') or []
        filters['branches'] = data.get('branches') or []
        filters['salepersons'] = data.get('salepersons') or []
        filters['currencies'] = data.get('currencies') or []

        if data.get('balance_is_zero'):
            filters['display_accounts'] = 'With balance is Zero'
    ...
    def build_where_clause(self, data=False):
        ...
        if data:
            ...
            if data.get('salepersons_ids', []):
                WHERE += ' AND (m.invoice_user_id IN %s' % str(tuple(data.get('salepersons_ids')) + tuple([0]))
                WHERE += ' OR l.enrt_sales_user_id IN %s)' % str(tuple(data.get('salepersons_ids')) + tuple([0]))

            if data.get('currency_ids', []):
                WHERE += ' AND (m.currency_id IN %s' % str(tuple(data.get('currency_ids')) + tuple([0]))
                WHERE += ' OR l.currency_id IN %s)' % str(tuple(data.get('currency_ids')) + tuple([0]))
    ...
    def build_detailed_move_lines(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        cr = self.env.cr
        data = self.get_filters(default_filters={})
        filter_currency_id = data.get('currency_ids')[0] if data.get('currency_ids') and len(data.get('currency_ids')) == 1 else False
        ...
        if ((count - offset_count) <= fetch_range) and data.get('initial_balance'):
            ...
            for row in cr.dictfetchall():
                ...
                row['partner_id'] = partner
                row['company_currency_id'] = currency_id.id
                row['account_currency_id'] = filter_currency_id
                move_lines.append(row)
        ...
        for partner in partner_ids:
            ...
            if self.branch_ids:
                sql = ('''
                    SELECT 
                        ...
                        COALESCE(SUM(l.credit),0) AS credit, 
                        COALESCE(SUM(l.debit - l.credit),0) AS balance,
                        COALESCE(SUM(CASE WHEN l.currency_id != l.company_currency_id THEN l.amount_currency ELSE 0 END),0) AS balance_in_fc
                    FROM account_move_line l
                    JOIN account_move m ON (l.move_id=m.id)
                    JOIN account_account a ON (l.account_id=a.id)
                    --LEFT JOIN account_account_type AS ty ON a.user_type_id = ty.id
            ...
            for row in cr.dictfetchall():
                if (not row['debit'] and not row['credit']) or (
                        ...
                        or (data.get('balance_is_zero') and (row['debit'] - row['credit']) != 0):
                    ...
                    move_lines[partner.id]['balance_in_fc'] = row['balance_in_fc']  # MK 23AUG24
    ...
    def get_filters(self, default_filters={}):
        ...
        branches = self.branch_ids if self.branch_ids else self.env['res.branch'].search(company_domain)
        ...
        filter_dict = {
            ...
            'balance_is_zero': self.balance_is_zero,
            ...
            'analytic_distribution_list': [(a.id, a.name) for a in self.env['account.analytic.account'].search([])],
            'analytic_distribution_ids': self.analytic_distribution_ids.ids,
            'salepersons': [(a.id, a.name) for a in self.env['res.users'].search([])],
            'salepersons_ids': self.salepersons_ids.ids,
            'currencies': [(a.id, a.name) for a in self.env['res.currency'].search([])],
            'currency_ids': self.currency_ids.ids,
            'branch_list': [(a.id, a.name) for a in branches],
    ...
    def action_xlsx(self):
        ...
        account_currency_obj = self.env['res.currency'].sudo()
        ...
        row_pos_2 += 2
        sheet_2.write(row_pos_2, 0, _('Branches'), format_header)
        j_list = ', '.join([lt[1] or '' for lt in filter.get('branch_list')])
        sheet_2.write(row_pos_2, 1, j_list, content_header)
        # Partners
        row_pos_2 += 1
        ...
        if filter.get('include_details', False):
            ...
            sheet.write(row_pos, 7, _('Balance'),
                        format_header)
            sheet.write_string(row_pos, 8, _('Balance in FC'), format_header)
        else:
        ...
        if account_lines:
            for line in account_lines:
                ...
                if account_lines[line].get('account_currency_id'):
                    account_currency_id = account_currency_obj.browse(account_lines[line].get('account_currency_id'))
                    if account_currency_id.position == 'before':
                        acc_balance_in_fc = account_currency_id.symbol + ' ' + str(
                            float(account_lines[line].get('balance_in_fc')))
                    else:
                        acc_balance_in_fc = str(
                            float(account_lines[line].get('balance_in_fc'))) + ' ' + account_currency_id.symbol
                else:
                    acc_balance_in_fc = float(account_lines[line].get('balance_in_fc'))

                sheet.write(row_pos, 8, acc_balance_in_fc, line_header)
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write(row_pos, 7, float(sub_line.get('balance')),
                                        line_header_light_initial)
                            sheet.write(row_pos, 8, float(0), line_header_light_initial)
                            ...
                            sheet.write(row_pos, 1, sub_line.get('lcode'),
                                        line_header_light)
                            sbl_account_name = sub_line.get('account_name').get(
                                next(iter(sub_line.get('account_name'))))
                            sheet.write(row_pos, 2, sbl_account_name or '', line_header_light)
                            ...
                            # if 'amount_currency' in sub_line and sub_line.get('currency_symbol') != sub_line.get(
                            #         'company_currency_symbol'):
                            #     sheet.write(row_pos, 8, float(sub_line.get('amount_currency')), line_header_light)
                            # else:
                            #     sheet.write(row_pos, 8, float(0), line_header_light)

                            if 'amount_currency' in sub_line and sub_line.get('currency_symbol') != sub_line.get('company_currency_symbol'):
                                if sub_line.get('currency_position') == 'before':
                                    amount = sub_line.get('currency_symbol') + ' ' + str(float(sub_line.get('amount_currency')))
                                else:
                                    amount = str(float(sub_line.get('amount_currency'))) + ' ' + sub_line.get('currency_symbol')
                                sheet.write(row_pos, 8, amount, line_header_light)
                            else:
                                sheet.write(row_pos, 8, float(0), line_header_light)
                        else:  # Ending Balance
                            row_pos += 1
                            ...
                            sheet.write(row_pos, 7, float(account_lines[line].get('balance')),
                                        line_header_light_ending)
                            if account_lines[line].get('balance_in_fc') and account_lines[line].get('account_currency_id') != account_lines[line].get('company_currency_id'):
                                if account_lines[line].get('account_currency_id'):
                                    account_currency_id = account_currency_obj.browse(account_lines[line].get('account_currency_id'))
                                    if account_currency_id.position == 'before':
                                        amount = account_currency_id.symbol + ' ' + str(float(account_lines[line].get('balance_in_fc')))
                                    else:
                                        amount = str(float(account_lines[line].get('balance_in_fc'))) + ' ' + account_currency_id.symbol
                                else:
                                    amount = float(account_lines[line].get('balance_in_fc'))
                                sheet.write(row_pos, 8, amount, line_header_light_ending)
                            else:
                                sheet.write(row_pos, 8, float(0), line_header_light_ending)
