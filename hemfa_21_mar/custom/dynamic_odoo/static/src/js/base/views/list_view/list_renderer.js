odoo.define('dynamic_odoo.ListRenderer', function (require) {
    ...
    patch(ListRenderer.prototype, 'studio.ListRenderer', {
        setup() {
            this._super();
            onMounted(() => {
                this.search_advance = [1, "1", "True", true].includes($(this.props.archInfo.__rawArch).attr("search_advance"));
                this.search_domain = {};
                if (this.search_advance) {
        ...
        onSearchAdvance: function (params) {
            ...
            if (group) {
                this.env.searchModel.deactivateGroup(group.id);
            }
            if (filters && filters.length) {
                this.env.searchModel.createNewFilters(filters);
            }
        },
        ...
        renderSearch: function (node) {
            const {activeFields, model} = this.props.list;
            const name = node.rawAttrs.name, $th = $('<th>'), field = {...this.fields[name], name: name},
                withoutFields = ["activity_exception_decoration"];
            this.fieldRender = {
                selection: {
            ...
            const fieldWidget = new widget(rootWidget, {...field, ...funcProps()});
            fieldWidget.appendTo($th.addClass("wFSearch"));
            return $th;
        },
        _renderHeaderSearch: function () {
            const columns = this.getColumns();
            let $tr = $('<tr class="searchAdvance">');
            if (this.hasSelectors) {
                $tr.append($("<th class='o_list_record_selector'></th>"))
            }
            $tr.append(_.map(columns, this.renderSearch.bind(this)));
            if (this.displayOptionalFields || this.activeActions.onDelete) {
                $tr.append($("<th class='o_list_actions_header'></th>"));
            }
            $(this.__owl__.refs.table).addClass("hasSearchAdvance").find("thead").append($tr);
        },
    });
