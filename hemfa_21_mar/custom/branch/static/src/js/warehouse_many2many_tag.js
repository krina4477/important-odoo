...
export class DeleteConfirmationDialog extends Many2ManyTagsField {
     async deleteTag(id) {
            console.log("====================c===c===s==s=", id);
            const tagRecord = await this.props.value.records.find((record) => record.id === id);

           const action = await this.orm.call('stock.warehouse', "get_product_branch_id", [[this.props.record.data.id]], {
               branch: tagRecord.data.id,
         });
         if (action){
           const dialogProps = {
                            body: this.env._t("This branch is available in locations. Are you sure that you want to remove this record?"),
...
registry.category("fields").add("delete_warehouse_many2many_tags", DeleteConfirmationDialog);
