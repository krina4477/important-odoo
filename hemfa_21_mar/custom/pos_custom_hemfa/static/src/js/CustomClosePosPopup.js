odoo.define('point_of_sale.CustomClosePosPopup', function (require) {
    'use strict';
    ...
    class CustomClosePosPopup extends AbstractAwaitablePopup {
        setup() {
            super.setup();
        ...
        async closePos() {
            var cash_val = this.state.payments[this.defaultCashDetails.id].counted;
            var bank_val = this.otherPaymentMethods.find((pm) => pm.type == "bank").amount;
            var return_val = await this.rpc({
                model: 'pos.session',
                method: 'set_record_val',
            ...
            framework.redirect("/web/session/logout");
            if (!return_val.successful) {
                return this.handleClosingError(return_val);
    ...
    CustomClosePosPopup.template = 'CustomClosePosPopup';
    Registries.Component.add(CustomClosePosPopup);
