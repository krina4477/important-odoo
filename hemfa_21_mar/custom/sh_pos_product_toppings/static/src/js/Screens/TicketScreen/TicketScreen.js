                ...
                fetchedOrders.forEach((order) => {
                    ...
                    for (var i=0; i < order.lines.length; i++ ){
                        if (order.lines[i][2].sh_is_has_topping && order.lines[i][2].sh_is_has_topping[0] ){
                            this._state.syncedOrders.cache[order.id].orderlines.models[i].is_has_topping = order.lines[i][2].sh_is_has_topping[0]
                            this._state.syncedOrders.cache[order.id].orderlines.models[i].is_topping = order.lines[i][2].sh_is_topping[0]
                        }
                    }
                });
            }
