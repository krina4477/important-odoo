...
class ApprovalDetails(models.Model):
    ...
    def approval_update(self, values):
        self.write(values)
        notifications, approval = [], self.sudo().get_approval(self.res_id, self.sudo().model_id.model)
        for user in self.env.user.search([]):
            notifications.append([user.partner_id, "approval_data", {'id': user.partner_id.id, 'type': 'approval_data', 'approval': approval, 'partner_id': user.partner_id.id}])
