...
class ReportAccountAgedReceivable(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        if self.env.user.has_group('account.group_account_user') and self.env.user.company_id.account_aged_receivable_auto_reconcile:
            options['buttons'].append({'name': _('Auto Reconcile'), 'action': 'partner_auto_reconcile_aged_receivable', 'sequence': 22})

    
    def partner_auto_reconcile_aged_receivable(self, options):
        ...
        for line in all_lines:
            ...
            if line.get('id') and ('partner_id' in str(p_id)) or ('"groupby": "partner_id"' in str(p_id)) and len(partner_val) >= 3:
                if isInt(partner_val[6]) and isInt(partner_val[6]) not in partner_list:
        ...
        for partner in partner_list:
            ...
            print("0000000000000", account_move_line_data)
