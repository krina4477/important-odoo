    ...
    def remove_entries(self):
        for record in self:
            ...
            if not period_journal:
                raise UserError(
                    _("You have to set the 'End  of Year Entries Journal' for this Fiscal Year which is set after generating opening entries from 'Generate Opening Entries'."))
            if period_journal.period_id.state == 'done':
                raise UserError(
                    _("You can not cancel closing entries if the 'End of Year Entries Journal' period is closed."))
