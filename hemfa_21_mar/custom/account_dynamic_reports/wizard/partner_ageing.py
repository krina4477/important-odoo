...
class InsPartnerAgeing(models.TransientModel):
    ...
    salepersons_ids = fields.Many2many('res.users','partner_align_res_users_rel')
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'analytic_distribution_ids': self.env['account.analytic.account'].search([]).ids,
            'salepersons': [(a.id, a.name) for a in self.env['res.users'].search([])],
            'salepersons_ids': self.salepersons_ids.ids,
            'branch_list': [(a.id, a.name) for a in self.env['res.branch'].search([])],
    ...
    def process_filters(self):
        ...
        filters['branch_list'] = data.get('branch_list') or []
    ...
    def process_detailed_data(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        if partner:
            ...
            if self.analytic_distribution_ids:
                sql += ' AND line_analytic.analytic_distribution_id IN %s' % str(tuple(self.analytic_distribution_ids.ids) + tuple([0]))
            if self.salepersons_ids:
                sql += ' AND m.invoice_user_id IN %s' % str(tuple(self.salepersons_ids.ids) + tuple([0]))
            if self.branch_ids:
                sql += ' AND m.branch_id IN %s' % str(tuple(self.branch_ids.ids) + tuple([0]))
            ...
            if self.branch_ids:
                sql = """
                        ...
                        LEFT JOIN
                            account_journal AS j ON l.journal_id = j.id
                        LEFT JOIN
                            res_branch AS b ON l.branch_id = b.id    
                        LEFT JOIN 
    ...
    def action_xlsx(self):
        ...
        b_list = ', '.join([lt[1] or '' for lt in filter.get('branch_list')])
        sheet_2.write(row_pos_2, 1, b_list, content_header)
        row_pos_2 += 1
