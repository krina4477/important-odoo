...
class access_management(models.Model):
    ...
    user_ids = fields.Many2many('res.users','access_management_users_rel_ah', 'access_management_id', 'user_id','Users',compute='_access_user_group',store=True)#'access_management_users_rel_ah', 'access_management_id', 'user_id', 
    access_groups = fields.Many2many('res.groups',string='groups')
    access_user_role = fields.Many2one('security.role',string='User Role' ,store=True)
    access_user_related = fields.Many2many(related='access_user_role.user_ids')
    user_from_view = fields.Many2many('res.users','access_management_users_from_view','access_management_id','user_id','Users ')


    @api.depends('access_user_role','user_from_view')
    def _access_user_group(self):
        list_users=[]
        for usr in self.access_user_related:
                if usr.id not in list_users:
                    list_users.append(usr.id)
        for usr_view in self.user_from_view:
                if usr_view.id not in list_users:
                    list_users.append(usr_view.id)
        self.user_ids= [(6,0,list_users)]
