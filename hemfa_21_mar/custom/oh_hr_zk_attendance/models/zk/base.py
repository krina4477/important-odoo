...
class ZK(object):
    ...
    def get_attendance(self,start_date='',end_date='',policy='all',get_userid=''):
        """
            ...
            while len(attendance_data) >= 8:
                ...
                if policy=='range':
                    ...
                    if start_date!='' and end_date=='':
                        date_obj = datetime.strptime(start_date+' 00:00:00', date_format)
                        
                        if timestamp.date() >= date_obj.date():
                            if get_userid!='':
                        ...
                        date_obj = datetime.strptime(start_date+' 00:00:00', date_format)
                        date_obj_end = datetime.strptime(end_date+' 00:00:00', date_format)
                        if timestamp.date() >= date_obj.date() and timestamp.date() <=date_obj_end.date():
                            if get_userid!='':
                                if get_userid==user_id:
                                    attendance = Attendance(user_id, timestamp, status, punch, uid)
                                    attendances.append(attendance)
