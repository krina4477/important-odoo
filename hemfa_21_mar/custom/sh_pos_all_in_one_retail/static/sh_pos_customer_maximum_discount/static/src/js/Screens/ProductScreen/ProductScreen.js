odoo.define('sh_pos_customer_maximum_discount.ProductScreen', function (require) {
    ...
    const ShProductScren = (ProductScreen) =>
        class extends ProductScreen {
            _onClickPay() {
                var self = this;
                const orderLines = self.env.pos.get_order().orderlines;
                const has_zero = orderLines.filter(orderline => orderline.price == 0);
                console.log("======================c=c=c=cc=c===s", has_zero);
                ...
                if (has_zero.length >0){

                 Gui.showPopup('ErrorPopup', {
                            title: 'Zero Price',
                            body: 'Cashier Can Not Validate This Order Because One Of The Product Price Is Zero',
                          });
                          return;
