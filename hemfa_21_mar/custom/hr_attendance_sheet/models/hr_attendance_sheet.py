...
class AttendanceSheet(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if rec.payslip_id:
                raise UserError(_(
                    'You cannot delete an attendance sheet has payslip!'))
    ...
    def check_date(self):
        for sheet in self:
            ...
            emp_sheets_filter = emp_sheets.filtered(lambda s: not s.payslip_id.refund_payslip_id)

            for emp_sheet in emp_sheets_filter:
    ...
    def get_public_holiday(self, date, emp):
        ...
        public_holidays = self.env['hr.public.holiday'].sudo().search(
            [('date_from', '<=', date), ('date_to', '>=', date),
             ('state', '=', 'active')])
        logging.info(">>>>>>ph>>"+str(public_holidays)+"_"+str(date))
