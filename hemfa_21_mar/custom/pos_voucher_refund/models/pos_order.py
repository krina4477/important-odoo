...
class PosOrder(models.Model):
    ...
    def get_coupon_values(self, amount, partner_id, name):
        ...
        vals = dict(
            ...
            expiry_date=str(datetime.today().date()+relativedelta(days=config_obj.get('refund_coupon_validity'))),
            voucher_val_type='amount',
            use_minumum_cart_value=False,
            is_partially_redemed=config_obj.get('refund_coupon_partial_redeem'),
            voucher_usage='both',
    ...
    def create_from_ui(self, orders, draft):
        ...
        for order in orders:
            order_obj = self.search([('pos_reference', '=', order.get('data').get('name'))])
            coupon_name = "Refund Voucher (%s)" % order_obj.name
            if order.get('data').get('to_voucher_refund'):
                if order.get('data').get('refund_voucher_amount') > 0:
                    vals = self.get_coupon_values(amount=order.get('data').get('refund_voucher_amount'), partner_id=order.get('data').get('partner_id', False), name=coupon_name)
                    try:
                        voucher_id = self.env['voucher.voucher'].create(vals)
                        voucher_history_obj = self.env['voucher.history'].sudo().search([('voucher_id','=',voucher_id.id),('transaction_type','=','credit')],limit=1)
                        if voucher_history_obj:
                            voucher_history_obj.pos_order_id = order_obj.id
                            voucher_history_obj.description = "Voucher Created at %s" % str(datetime.today())
                        if order_ids[0] and order_obj.id == order_ids[0].get('id'):
                            if order_ids[0].get('voucher_ids'):
                                order_ids[0]['voucher_ids'].append(voucher_id.id)
                            else:
                                order_ids[0]['voucher_ids'] = [voucher_id.id]
...
class VoucherVoucher(models.Model):
    ...
    def _validate_n_get_value(self, secret_code, wk_order_total, product_ids, refrence=False, partner_id=False):
        ...
        if self_obj.customer_type == 'special_customer':
            if (self_obj.is_partially_redemed != True) and (len(used_vouchers) >= 1):
                result['type'] = _('ERROR')
