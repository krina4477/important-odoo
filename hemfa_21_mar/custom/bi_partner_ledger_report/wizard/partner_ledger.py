...
class Accounting_reportPartner_ledger(models.TransientModel):
    ...
    def print_partner_ledger(self):
        ...
        params = [tuple(data['computed']['move_state']),tuple(select_companies.ids), tuple(data['computed']['account_ids'])] + query_get_data[2]
        ...
        if self._context.get('report_type') != 'excel':
            ...
            for partner in docs:
                ...
                if final_data:
                    extra_debit =  final_data[0].get('debit',0.0) or 0.0
                    extra_credit = final_data[0].get('credit',0.0) or 0.0
                    extra_balance = final_data[0].get('initial_balance',0.0) or 0.0
                else:
                    extra_debit =   0.0
                    extra_credit =  0.0
                    extra_balance =  0.0
                ...
                debit = self.env['report.bi_partner_ledger_report.bi_report_partnerledger'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(data, partner,
                                                                                        'debit')
                if final_data:
                    debit = debit + extra_debit
                else:
                    debit = debit
