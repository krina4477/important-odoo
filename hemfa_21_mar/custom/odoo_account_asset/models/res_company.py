...
class Company(models.Model):
    ...
    def custom_compute_fiscalyear_dates(self, current_date):
        ...
        fiscalyear = self.env['custom.account.fiscal.year'].search([
            ('company_id', '=', self.id),
            ('date_from', '<=', date_str),
            ('date_to', '>=', date_str),
        ], limit=1)
        ...
        fiscalyear_from = self.env['custom.account.fiscal.year'].search([
            ('company_id', '=', self.id),
        ...
        fiscalyear_to = self.env['custom.account.fiscal.year'].search([
            ('company_id', '=', self.id),
