odoo.define('dynamic_odoo.tree', function (require) {
    ...
    var ListEditRenderer = ListRenderer.extend({
        init: function (parent, state, params) {
            ...
            // this.fieldDecorations['undefined'] = {};
        },
        ...
        _renderRow: function (record, index) {
            const activity_state = record.fields.activity_state;
            if (activity_state && !activity_state.selection) {
                activity_state.selection = [
                    ('overdue', 'Overdue'),
                    ('today', 'Today'),
                    ('planned', 'Planned')]
            }
            return this._super(record, index);
        },
        ...
        _processColumns: function (columnInvisibleFields) {
            ...
            const {show_invisible} = this.arch.attrs, showInvisible = [1, '1', true, 'true', 'True'].includes(show_invisible);
            ...
            _.each(this.arch.children, function (c) {
                if (c.tag !== 'control' && c.tag !== 'groupby' && c.tag !== 'header') {
                    ...
                    if (showInvisible && reject && c.tag == 'field') {
                        self.columns.push(c);
                        c.attrs.virtual_invisible = true;
                    }
    ...
    var ListProps = baseEdit.ViewProps.extend({
        init: function (parent, props) {
            ...
            this.nodeProps.invisible.prepareProps = this.prepareInvisibleProps.bind(this);
        ...
        prepareInvisibleProps: function (node, nodeProps) {
            const {column_invisible} = node.attrs.modifiers || {};
            if (!('invisible' in (node.attrs.modifiers || {})) && 'column_invisible' in (node.attrs.modifiers || {})) {
                nodeProps.value = column_invisible;
            }
        },
    ...
    var ListEditContent = baseEdit.ViewContent.extend({
        ...
        prepareDataToSave: function () {
            ...
            this.props.viewInfo.arch.children.map((node) => {
                delete node.attrs.modifiers;
            });
            return this._super();
        },
