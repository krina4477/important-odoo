...
class ShSaleOrder(models.Model):
    ...
    def _compute_amount_word(self):
        ...
        lang = tools.get_lang(
            self.env, self.partner_id.lang or self.env.user.lang)
        amount_words = tools.ustr('{amt_value} {amt_word}').format(
            amt_value=_num2words(integer_value, lang=lang.iso_code),
            amt_word=_(currency.currency_unit_label) or "",
        )
        ...
        if not currency.is_zero(amount - integer_value):
            amount_words += ' ' + tools.ustr('{amt_separator} {amt_value} {amt_word}').format(
                ...
                amt_word=_(currency.currency_subunit_label) or "",

            )
        amount_words += ' ' + \
            tools.ustr('{amt_cft}').format(
                amt_cft=_(currency.close_financial_text) or "")
