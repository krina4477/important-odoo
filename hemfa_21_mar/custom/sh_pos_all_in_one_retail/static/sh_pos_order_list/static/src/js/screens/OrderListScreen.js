...
//        }
        ...
        if (query === '') {
            // Show all order lines if the query is empty
            $('.sh_order_line').show();
        } else {
            // Filter and display order lines based on the query
            console.log("All Orders:", this.env.pos.db.all_display_order);

            // Iterate over all orders
            this.env.pos.db.all_display_order.forEach(order_line => {
                let cleanedOrder = order_line.pos_reference.replace(/Order|[\s-]/g, '');
                let isMatch = cleanedOrder && cleanedOrder.indexOf(query) !== -1;

                $('.sh_order_line').each(function() {
                    // Show or hide order lines based on match
                    if (order_line.id == $(this).data('id')) {
                        $(this).toggle(isMatch);
                    }
                });
            });
        ...
        print_pos_order(event) {
            ...
            _.each(order_data.lines, function (each_order_line) {
                ...
                if (product) {
                    current_order.add_product(product, {
                        quantity: line_data.secondary_qty,
                        price: line_data.price_unit,
                        discount: line_data.discount,
                        merge: false,
                    });
                }
                 var secondaryUom = self.env.pos.units_by_id[line_data.secondary_uom_id[0]]
