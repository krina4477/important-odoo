...
class HrPayslip(models.Model):
    ...
    def _check_dates(self):
        ...
        for payslip in self:
            if payslip.credit_note:
                continue
            emp_payslips = self.env['hr.payslip'].search(
                [
            ...
            for emp_payslip in emp_payslips:
                if payslip.id == emp_payslip.id:
                    continue
                if max(payslip.date_from, emp_payslip.date_from) < min(
                        payslip.date_to, emp_payslip.date_to):
                    raise UserError(_(
                        'You Have Already Payslip For That Period!'))
    ...
    refund_payslip_id = fields.Many2one('hr.payslip')

    def refund_sheet(self):
        for payslip in self:
            if payslip.remain_amount != payslip.amount:
                raise ValidationError(_("Cannot refund paid payslip."))

            copied_payslip = payslip.copy({'credit_note': True, 'name': _('Refund: ') + payslip.name})
            payslip.refund_payslip_id = copied_payslip.id
