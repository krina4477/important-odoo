...
class CTXAccountAssetCategory(models.Model):
    _name = 'ctx.account.asset.category'
    _description = 'Asset/Revenue Recognition category'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'analytic.mixin']

    active = fields.Boolean(default=True)
    name = fields.Char(required=True, index=True, string="Asset Type Name")
    ...
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')], tracking=True,
                              string='Computation Method', required=True, default='linear',
                              help="Choose the method to use to compute the amount of depreciation lines.\n"
                                   "  * Linear: Calculated on basis of: Gross Value / Number of Depreciations\n"
                                   "  * Degressive: Calculated on basis of: Residual Value * Degressive Factor")
    method_number = fields.Integer(string='Number of Depreciations', default=5, tracking=True,
                                   help="The number of depreciations needed to depreciate your asset")
    method_period = fields.Integer(string='Period Length', default=1, tracking=True,
    ...
    method_time = fields.Selection([('number', 'Number of Entries'), ('end', 'Ending Date')],
                                   string='Time Method', required=True, default='number', tracking=True,
    ...
    open_asset = fields.Boolean(string='Auto-Confirm Assets', tracking=True,
                                help="Check this if you want to automatically confirm the assets "
                                     "of this category when created by invoices.")
    group_entries = fields.Boolean(string='Group Journal Entries', tracking=True,
    ...
    date_first_depreciation = fields.Selection([
        ...
        string='Depreciation Dates', default='manual', required=True, tracking=True,
    ...
    def _onchange_account_depreciation_id(self):
        ...
        if self.type == 'purchase' and not self.account_asset_id:
    ...
    def _onchange_account_depreciation_expense_id(self):
        if self.type not in ('purchase', 'expense'):
...
class CTXAccountAsset(models.Model):
    _name = 'ctx.account.asset'
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True,
                                 ...
                                 default=lambda self: self.env.company)
    note = fields.Text()
    category_id = fields.Many2one('ctx.account.asset.category', string='Category',
    ...
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')],
                              ...
                              help="Choose the method to use to compute the amount of depreciation lines.\n  * Linear:"
                                   ...
                                   "  * Degressive: Calculated on basis of: Residual Value * Degressive Factor")
    method_number = fields.Integer(string='No. of Depreciations', readonly=True,
                                   states={'draft': [('readonly', False)]}, default=5,
                                   help="The number of depreciations needed to depreciate your asset")
    method_period = fields.Integer(string='No. of Months', required=True,
    ...
    depreciation_line_ids = fields.One2many('ctx.account.asset.depreciation.line', 'asset_id',
    ...
    book_value = fields.Monetary(string='Book Value', readonly=True, compute='_compute_book_value', recursive=True,
                                 store=True,
    ...
    def unlink(self):
        ...
        return super(CTXAccountAsset, self).unlink()

    @api.model
    def _cron_generate_entries(self):
        self.compute_ctx_generated_entries(datetime.today())

    @api.model
    def compute_ctx_generated_entries(self, date, asset_type=None):
        ...
        ungrouped_assets = self.env['ctx.account.asset'].search(
        ...
        for grouped_category in self.env['ctx.account.asset.category'].search(
                type_domain + [('group_entries', '=', True)]):
            assets = self.env['ctx.account.asset'].search(
            ...
            for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                ...
                vals = {
                    ...
                    'depreciated_value': self.value - (
                            self.salvage_value + residual_amount + self.already_depreciated_amount_import),
    ...
    def validate(self):
        ...
        ref_tracked_fields = self.env['ctx.account.asset'].fields_get(fields)
        for asset in self:
    ...
    def _get_disposal_moves(self, invoice_line_ids, disposal_date):
        def get_line(asset, amount, account):
            return (0, 0, {
                ...
                'analytic_distribution': analytic_distribution if asset.category_id.type == 'sale' else {},
        ...
        for asset, invoice_line_id in zip(self, invoice_line_ids):
            ...
            if unposted_depreciation_line_ids:
                ...
                depreciation_moves = asset.depreciation_line_ids.filtered(lambda x: x.move_check)
                depreciated_amount = copysign(
                    sum(depreciation_moves.mapped('amount')) + asset.already_depreciated_amount_import, -initial_amount)
                depreciation_account = asset.category_id.account_depreciation_id
                for invoice_line in invoice_line_ids:
                    dict_invoice[invoice_line.account_id] = copysign(invoice_line.price_subtotal,
                ...
                commands.append((0, False, vals))
                asset.write({'depreciation_line_ids': commands, 'method_end': disposal_date, 'method_number': sequence})
                tracked_fields = self.env['ctx.account.asset'].fields_get(['method_number', 'method_end'])
                ...
                asset.depreciation_line_ids[-1].move_id = move_id.id
                move_ids += [move_id.id]
    ...
    def action_set_to_close(self):
        ...
        self.ensure_one()
        new_wizard = self.env['ctx.account.asset.sell'].create({
            'asset_id': self.id,
        })
        return {
            'name': _('Sell Asset'),
            'view_mode': 'form',
            'res_model': 'ctx.account.asset.sell',
            'type': 'ir.actions.act_window',
            'target': 'new',
    ...
    @api.depends('value', 'salvage_value', 'already_depreciated_amount_import', 'depreciation_line_ids.move_check',
    ...
    def _amount_residual(self):
        for rec in self:
            ...
            rec.value_residual = rec.value - total_amount - rec.salvage_value - rec.already_depreciated_amount_import
    ...
    def _entry_count(self):
        for asset in self:
            res = self.env['ctx.account.asset.depreciation.line'].search_count(
    ...
    def onchange_category_id_values(self, category_id):
        if category_id:
            category = self.env['ctx.account.asset.category'].browse(category_id)
            return {
                'value': {
    ...
    def copy_data(self, default=None):
        ...
        return super(CTXAccountAsset, self).copy_data(default)

    def _compute_entries(self, date, group_entries=False):
        depreciation_ids = self.env['ctx.account.asset.depreciation.line'].search([
    ...
    def create(self, vals):
        asset = super(CTXAccountAsset, self.with_context(mail_create_nolog=True)).create(vals)
        asset.sudo().compute_depreciation_board()
        return asset

    def write(self, vals):
        res = super(CTXAccountAsset, self).write(vals)
...
class AccountAssetDepreciationLine(models.Model):
    _name = 'ctx.account.asset.depreciation.line'
    _description = 'Asset Depreciation/Revenue Recognition line'

    name = fields.Char(string='Depreciation/Recognition Name', required=True, index=True)
    sequence = fields.Integer(required=True)
    asset_id = fields.Many2one('ctx.account.asset', string='Asset',
    ...
    def _prepare_move(self, line):
        ...
        move_line_1 = {
            ...
            'analytic_distribution': analytic_distribution if category_id.type == 'sale' else {},
        ...
        move_line_2 = {
            ...
            'partner_id': line.asset_id.partner_id.id,
            'analytic_distribution': analytic_distribution if category_id.type == 'purchase' else {},
            'currency_id': company_currency != current_currency and current_currency.id or company_currency.id,
            'amount_currency': line.amount,
        }
        move_vals = {
            'ref': line.asset_id.code or line.asset_id.name,
