...
class productPricelistItem(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            name = rec.name
            if rec.uom_id:
                name = str(name) + '-' + str(rec.uom_id.name)
            result.append((rec.id, name))
        return result


class BarcodeProductLines(models.TransientModel):
    _name = "barcode.product.lines.multi"
    _inheirt = "barcode.product.lines"
    _description = 'barcode.product.lines.multi'
    ...
    # @api.depends('product_id', 'product_pricelist_item_id')
    @api.onchange('product_id', 'product_pricelist_item_id')
    def onchange_product(self):
        print('ffffffffffffffffffff')
        if not self.product_id.id:
            self.pricelist_id = self.price = self.uom_id = self.product_pricelist_item_id = self.barcode = False
            return {}
        if self.product_pricelist_item_id:
            pricelist_item = self.product_pricelist_item_id
            self.pricelist_id = pricelist_item.pricelist_id.id
            self.price = pricelist_item.fixed_price
            self.uom_id = pricelist_item.uom_id.id

            multi_barcode = self.env['product.template.barcode'].search([
                ('product_id', '=', self.product_id.id), ('available_item', '=', True),
                ('price_lst', '=', pricelist_item.pricelist_id.id), ('unit',
                                                                     '=', pricelist_item.uom_id.id)
            ])

            if multi_barcode:
                self.barcode = multi_barcode[0].name
            else:
                self.barcode = False
        return {
            'domain':
...
class BarcodeLabels(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        view_id = self.env['ir.ui.view'].search(
            [('name', '=', 'report_barcode_custom')])
        print(">>>>>>>>>>>>>>>>>>>>", view_id, view_id.arch)
        if not view_id.arch:
    ...
    def print_report(self):
        ...
        datas = {
            ...
            'form': {
                ...
                'product_ids': [{
                    ...
                    'price': line.price,
                    'product_pricelist_item_id': line.product_pricelist_item_id,
                    'uom_id': line.uom_id,
        ...
        for line in self.product_get_ids:
            barcode_value = line.barcode  # product[config_rec.barcode_field]
            if not barcode_value:
                raise Warning(_('Please define barcode for %s!' % (
                    # product['name']
        ...
        return self.env.ref('dynamic_barcode_labels_custom.barcodelabels').report_action([], data=datas)
