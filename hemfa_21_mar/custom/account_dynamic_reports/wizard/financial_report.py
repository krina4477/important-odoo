...
class InsFinancialReport(models.TransientModel):
        ...
        if accounts:
            if self.account_report_id != \
                ...
                if self.analytic_distribution_ids:
                    domain = [('analytic_distribution_ids','in',self.analytic_distribution_ids.ids)]
                if self.branch_ids:
                    domain = [('move_id.branch_id','in',self.branch_ids.ids)]
                tables, where_clause, where_params = self.env['account.move.line'].with_context(context)._query_get(domain)
            else:
    ...
    def get_report_values(self):
        ...
        branch_ids = self.branch_ids or self.env['res.branch'].search(company_domain)
        ...
        data['form'] = self.read(
            ['date_from', 'enable_filter', 'debit_credit', 'date_to', 'date_range',
             ...
             'analytic_ids', 'strict_range','journal_ids',
        ...
        data['form'].update({'branch_list': [(j.id, j.name) for j in branch_ids]})
        data['form'].update({'analytics_list': [(j.id, j.name) for j in analytics]})
        data['form'].update({'analytic_distribution_list': [(a.id, a.name) for a in self.env['account.analytic.account'].search([])]})
        # data['form'].update({'branches': [(a.id, a.name) for a in self.env['res.branch'].search([])]})
        ...
        used_context['branch_ids'] = self.branch_ids.ids
        ...
        comparison_context['branch_ids'] = self.branch_ids.ids
    ...
    analytic_distribution_ids = fields.Many2many('account.analytic.account','fin_line_analytic_distribution_rel')
    ...
    def action_xlsx(self):
        ...
        if data['form'].get('date_to'):
            sheet2.write(row_pos_2, 1, datestring,
                                        content_header_date)
        row_pos_2 += 1
        sheet2.write(row_pos_2, 0, _('Branches'), format_header)
        branch_list = ', '.join([ lt[1] or '' for lt in data['form']['branch_list']])
        sheet2.write(row_pos_2, 1, branch_list,
                     content_header_date)
        row_pos_2 += 1
