odoo.define('dynamic_odoo.kanban', function (require) {
    ...
    var KanBanContentRecord = KanbanRecord.extend({
        ...
        _transformRecord: function (recordData) {
            ...
            for (const name of this.state.getFieldNames()) {
                ...
                var formatter = r.type ? field_utils.format[r.type] : _.identity;
                if (formatter == undefined && sFormatters[r.type]) {
                    formatter = sFormatters[r.type];
                }
                r.raw_value = value;
    ...
    var KanBanViewContent = baseEdit.ViewContent.extend({
        ...
        jsonToXml: function (arch) {
            ...
            const needRemoveTF = [];
            this.findInNodeTF(arch, (n) => (n.attrs && n.attrs['t-if'] || "").includes("kanban_compute_domain"), needRemoveTF);
            needRemoveTF.map((node) => {
        ...
        findInNodeTF: function (node, predicate, result) {
            if (predicate(node)) {
                result.push(node);
            }
            if (!node.children) {
            ...
            for (var i = 0; i < node.children.length; i++) {
                this.findInNodeTF(node.children[i], predicate, result);
            }
        },
