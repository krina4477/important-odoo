            ...
            async _clickProduct(event) {
                var self = this;
                    ...
                    await _.each(self.env.pos.db.product_by_id,function (each_product) {
                        var product_tmpl_id = 0
                        if(typeof(each_product.product_tmpl_id) == "object"){
                            product_tmpl_id = each_product.product_tmpl_id[0]
                        }else{
                            product_tmpl_id = each_product.product_tmpl_id
                        }
                        if ((product_tmpl_id == event.detail.product_tmpl_id) || (event.detail.product_tmpl_id[0] && event.detail.product_tmpl_id[0] == product_tmpl_id)) {
                    ...
                    if (this.product_variants && this.product_variants.length > 1) {
                        if (!self.env.pos.config.sh_pos_variants_group_by_attribute && self.env.pos.config.sh_pos_enable_product_variants) {
                            ...
                            if (this.Attribute_names && this.Attribute_names.length > 0) {
                        ...
                        if (this.alternative_products && this.alternative_products.length > 0 && self.env.pos.config.sh_pos_display_alternative_products && self.env.pos.config.sh_pos_variants_group_by_attribute) {
