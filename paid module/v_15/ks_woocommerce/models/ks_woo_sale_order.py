...
class KsStockQuant(models.Model):
    ...
    @api.model
    def create(self, vals):
        record = super(KsStockQuant, self).create(vals)
        if vals.get('quantity') and self.quantity != vals.get('quantity'):
    ...
    def write(self, vals):
        ...
        if vals.get('quantity') and self.quantity != vals.get('quantity') and self.location_id.usage == 'internal':
            super(KsStockQuant, self).write(vals)
    ...
    def update_stock_in_woo(self):
        ...
        if stock_id:
            ...
            if product_template.ks_product_template:
                for layers in product_template.ks_product_template:
                    instance_id = layers.ks_wc_instance
                    if instance_id.ks_auto_stock_sync_to_woo:
                        _logger.info("""Enqueuing Product""")
                        self.env['ks.woo.queue.jobs'].ks_create_product_stock_record_in_queue(instance_id,
                                                                                              records=product_template.ks_product_template)
                    else:
                        _logger.info("""Instance %s is not configured to Auto Stock Sync.""" % (
                            str(instance_id.display_name)))
            else:
