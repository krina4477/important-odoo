...
class ProductTempalte(models.Model):
    ...
    prod_var = fields.Boolean(
        "Compute Product Variant", compute="_compute_set_product_template")

    def _compute_set_product_template(self):
        self.prod_var = False


class ShCustomModelProductTemplate(models.Model):
    _name = 'sh.custom.model.product.template'
    _description = 'Custom Model Product template'
    ...
    def set_domain(self):
        ...
        view_id = self.env.ref('product.product_template_form_view')
    ...
    def unlink(self):
        ...
        res = super(ShCustomModelProductTemplate, self).unlink()
    ...
    def create(self, vals):
        ...
        return super(ShCustomModelProductTemplate, self).create(vals)
    ...
    def write(self, vals):
        if vals.get('field_type'):
            if vals.get('field_type') == 'color':
                ...
                vals.update({'ttype': vals.get('field_type')})
        res = super(ShCustomModelProductTemplate, self).write(vals)
        ...
        inherit_id = self.env.ref('product.product_template_form_view')

        if self.ir_ui_view_obj:  # update view if exist
            self.ir_ui_view_obj.write({'name': 'product.template.dynamic.fields',
                                       'type': 'form',
                                       'model': 'product.template',
                                       'mode': 'extension',
    ...
    def create_fields(self):
        ...
        irui_vew_obj = self.env['ir.ui.view'].create({'name': 'product.template.dynamic.fields',
                                                      'type': 'form',
                                                      'model': 'product.template',
                                                      'mode': 'extension',
                                                      'inherit_id': inherit_id.id,
...
#                     elif 'name' in tag.attrib and not 'string' in tag.attrib:
#                         tab_list.insert(len(tab_list),(tag.attrib['name'], tag.attrib['name']))
#                     if tag.attrib['name'] and tag.attrib['string']:
...
### Fields Declaration
    ...
    groups = fields.Many2many(
        'res.groups', 'sh_custom_group_product_template_rel', 'field_id', 'group_id')
    sh_selection_ids = fields.One2many("sh.model.fields.product.template.selection",
                                       "sh_field_product_template_id", string="Selection Option", copy=True)
...
class FieldSelection(models.Model):
    _name = 'sh.model.fields.product.template.selection'
    _description = 'Field Selection'

    sh_field_product_template_id = fields.Many2one(
        'sh.custom.model.product.template', string="SH Custom Product Template")
