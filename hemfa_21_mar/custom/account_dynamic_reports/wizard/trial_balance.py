...
class InsTrialBalance(models.TransientModel):
    ...
    display_accounts = fields.Selection(
        [('all', 'All'),
         ('balance_not_zero', 'With balance not zero'),('balance_is_zero','With balance is zero')], string='Display accounts',
    )
    ...
    analytic_distribution_ids = fields.Many2many('account.analytic.account','trial_line_analytic_distribution_rel')
    ...
    def process_filters(self, data):
        ...
        if data.get('branch_ids', []):
            filters['branch_list'] = self.env['res.branch'].browse(data.get('branch_ids', [])).mapped('name')
        else:
            filters['branch_list'] = ''
    ...
    def process_data(self, data):
        if data:
            ...
            if data.get('analytic_distribution_ids', []):
                WHERE += ' AND line_analytic.analytic_distribution_id IN %s' % str(tuple(data.get('analytic_distribution_ids')) + tuple([0]))
            if self.branch_ids:
                if data.get('branch_ids', []):
