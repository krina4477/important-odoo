...
class MaterialPurchaseRequisition(models.Model):
    ...
    def confirm_requisition(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_to_res_id('material_purchase_requisitions.email_employee_purchase_requisition')
    ...
    def department_approve(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_to_res_id('material_purchase_requisitions.email_manager_purchase_requisition')
            email_template_obj = self.env['mail.template'].sudo().browse(template_id)
            if template_id:
    ...
    def action_approve(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_to_res_id('material_purchase_requisitions.email_user_purchase_requisition')
            email_template_obj = self.env['mail.template'].sudo().browse(template_id)
            if template_id:
                values = email_template_obj._generate_template(requisition.ids,
                                                               ['subject', 'body_html', 'email_from', 'email_to',
