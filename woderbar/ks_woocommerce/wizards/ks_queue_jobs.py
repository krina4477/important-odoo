            ...
            if record.ks_model == 'stock':
                ...
                try:
                    if record.ks_operation == 'woo_to_odoo':
                        ...
                        inventory_adjustment_created = self.env['stock.quant'].ks_create_stock_inventory_adjustment(
                ...
                try:
                    if record.ks_operation == 'woo_to_odoo':
                        ...
                        if order_record_exist:
                            order_record_exist.ks_woo_import_order_update(order_data, queue_record=record, update=record.ks_order_update)
                        else:
                            if not record.ks_order_update:
                                ...
                                self.env['ks.woo.logger'].ks_create_odoo_log_param(ks_operation_performed="update_odoo",
                                                                                   ...
                                                                                   ks_message="Import the Order before updating the status!",
                                                                                   ks_status="failed",
                                                                                   ks_type="order",
                                                                                   ks_record_id=0,
    ...
    def ks_create_product_record_in_queue(self, instance=False, data=False, records=False, product_config=False,
        ...
        if data:
            if instance.ks_sync_unpublished_product:
                for record in data:
                    if record.get('status') == 'pending' or record.get('status') == 'private' or record.get('status') == 'publish' or record.get('status') == 'draft':
                        ks_woo_id = record.get('id')
                        product_data = {
                            'ks_name': record.get('name'),
                            'ks_wc_instance': instance.id,
