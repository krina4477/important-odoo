...
class AccountInvoice(models.Model):
    ...
    def _set_total_progress_bill(self):
        for rec in self:
            rec.total_progress_billing = rec.analytic_account_id.total_progress_account

    # @api.multi
    #    @api.depends('amount_total')
    @api.depends('analytic_account_id',
                 'amount_total')  # 13 Add project due to calculate invoice_to_date amount if project change
    def _set_invoiceto_date(self):
        for rec in self:
            rec.invoice_to_date = 0.0
            #            cus_inv = self.search([('state', 'in', ['open', 'paid']), ('partner_id', '=', rec.partner_id.id), ('analytic_account_id', '=', rec.analytic_account_id.id)])
            if rec.analytic_account_id:  # odoo13
                cus_inv = self.search(['|', ('state', 'in', ['posted']), ('invoice_payment_state', 'in', ['paid']),
                                       ('partner_id', '=', rec.partner_id.id), ('analytic_account_id', '=', rec.analytic_account_id.id),
                                       ('type', '=', 'out_invoice')])
                for inv in cus_inv:
    ...
    @api.depends('analytic_account_id', 'amount_total', 'amount_residual')  # 13 Add project due to calculate previously_invoice
    def _set_previously_invoiced(self):
        for rec in self:
            #            pre_inv = self.search([('state', 'in', ['open', 'paid']), ('partner_id', '=', rec.partner_id.id), ('analytic_account_id', '=', rec.analytic_account_id.id)])
            rec.previously_invoice = 0.0
            rec.previously_invoice_due = 0.0
            if rec.analytic_account_id:  # Search projects if project on invoice
                pre_inv = self.search(['|', ('state', 'in', ['posted']), ('invoice_payment_state', 'in', ['paid']),
                                       ('partner_id', '=', rec.partner_id.id), ('analytic_account_id', '=', rec.analytic_account_id.id),
                                       ('type', '=', 'out_invoice')])
