odoo.define('account_dynamic_reports.DynamicTbMain', function (require) {
    ...
    var DynamicFrMain = AbstractAction.extend({
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                if (initial_render) {
                    ...
                    self.$el.find('.branch-multiple').select2({
                    ...
                    self.$el.find('.analytic_accounts-multiple').select2({
                    ...
                    self.$el.find('.sale_person-multiple').select2({
        ...
        view_gl: function (event) {
            ...
            var branch_ids = [];
            var branch_list = $(".branch-multiple").select2('data')
            for (var i = 0; i < branch_list.length; i++) {
                journal_ids.push(parseInt(branch_list[i].id))
            }
            domains.branch_ids = branch_ids
            var analytic_ids = [];
        ...
        update_with_filter: function (event) {
            ...
            var analytic_distribution_ids = [];
            var analytic_distribution_list = $(".analytic_accounts-multiple").select2('data')
            for (var i = 0; i < analytic_distribution_list.length; i++) {
                analytic_distribution_ids.push(parseInt(analytic_distribution_list[i].id))
            }
            output.analytic_distribution_ids = analytic_distribution_ids
            ...
            // }
            // output.branch_ids = branch_ids

//             var salepersons_ids = [];
//             var salepersons_list = $(".sale_person-multiple").select2('data')
//             for (var i = 0; i < salepersons_list.length; i++) {
//                 salepersons_ids.push(parseInt(salepersons_list[i].id))
//             }
//             output.salepersons_ids = salepersons_ids
    ...
    var DynamicGlMain = AbstractAction.extend({
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                _.each(self.account_data, function (k, v) {
                    ...
                    var formatOptionsAcc = {
                        currency_id: k.account_currency_id != false ? k.account_currency_id : k.company_currency_id,
                        noSymbol: true,
                    };
                    k.debit = self.formatWithSign(k.debit, formatOptions, k.debit < 0 ? '-' : '');
                    k.credit = self.formatWithSign(k.credit, formatOptions, k.credit < 0 ? '-' : '');
                    k.balance = self.formatWithSign(k.balance, formatOptions, k.balance < 0 ? '-' : '');
                    if (k.account_currency_id != false){
                        k.balance_in_fc = self.formatWithSign(k.balance_in_fc, formatOptionsAcc, k.balance_in_fc < 0 ? '-' : '');
                    }else{
                        k.balance_in_fc = k.balance_in_fc
                    }
                ...
                if (initial_render) {
                    ...
                    self.$el.find('.currency-multiple').select2({
        ...
        update_with_filter: function (event) {
            ...
            output.balance_is_zero = false;
            output.display_accounts = false;
            ...
            for (var i = 0; i < options_list.length; i++) {
                ...
                if (options_list[i].id === 'balance_is_zero') {
                    output.balance_is_zero = true;
                }
            }
            if ($("#date_from").val()) {
                var dateObject = $("#date_from").datepicker("getDate");
