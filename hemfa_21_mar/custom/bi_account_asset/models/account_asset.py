...
class AccountAssetCategory(models.Model):
    ...
    account_asset_id = fields.Many2one('account.account', string='Asset Account', required=True, domain=[('account_type','=','income_other'), ('deprecated', '=', False)], help="Account used to record the purchase of the asset at its original price.")
    account_depreciation_id = fields.Many2one('account.account', string='Depreciation Entries: Asset Account', required=True, domain=[('account_type','=','income_other'), ('deprecated', '=', False)], help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Depreciation Entries: Expense Account', required=True, domain=[('account_type','=','income_other'), ('deprecated', '=', False)], oldname='account_income_recognition_id', help="Account used in the periodical entries, to record a part of the asset as expense.")
    ...
    def _valid_field_parameter(self, field, name):
        return name == 'oldname' or super()._valid_field_parameter(field, name)
...
class AccountAssetAsset(models.Model):
    ...
    def _valid_field_parameter(self, field, name):
        return name == 'oldname','method' or super()._valid_field_parameter(field, name)
    ...
    def _get_disposal_moves(self):
        ...
        for asset in self:
            ...
            if unposted_depreciation_line_ids:
                ...
                print('********============> changes, tracking_value_ids', changes, tracking_value_ids)
                if changes:
    ...
    def create(self, vals_list):
        asset = super(AccountAssetAsset, self.with_context(mail_create_nolog=True)).create(vals_list)
...
class AccountAssetDepreciationLine(models.Model):
    ...
    def _valid_field_parameter(self, field, name):
        return name == 'ondelete','track_visibility' or super()._valid_field_parameter(field, name)
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            ...
            move_line_1 = {
                ...
                'analytic_distribution': category_id.analytic_distribution if category_id.type == 'sale' else False,
            ...
            move_line_2 = {
                ...
                'journal_id': category_id.journal_id.id,
                'partner_id': line.asset_id.partner_id.id,
                'analytic_distribution': category_id.analytic_distribution if category_id.type == 'purchase' else False,
                'currency_id':  current_currency.id if current_currency else False,
                'date_maturity' : depreciation_date or False,
            }
            move_vals = {
