                ...
                if records_stock_move_line:
                    ...
                    if self.env.company.sudo().sh_inventory_barcode_scanner_is_add_product:
                        ...
                        if search_product:
                            ...
                            line_val = {
                                'product_id': search_product_code.id,
                                'location_dest_id': self.location_dest_id.id,
                                'qty_done': 1,
                                'location_id': self.location_id.id,
                                'sh_inventory_barcode_scanner_is_last_scanned': is_last_scanned,
                            ...
                            if search_product_code.unit:
                                line_val.update({
                                    "product_uom_id": search_product_code.unit.id,
                                })
                            self.update({
                                field_name_stock_move_line: [(0, 0, line_val)]
                            })
            ...
            # =================================================================
                ...
                if search_mls:
                    ...
                    if self.env.company.sudo().sh_inventory_barcode_scanner_is_add_product:
                        ...
                        search_product_code = self.env["product.template.barcode"].sudo().search(
                            [('name', '=', barcode),
                                ('available_item', '=', True)], limit=1)

                        if search_product:
                            move_search_product_code = self.move_ids_without_package.sudo().filtered(
                                lambda l: l.product_id.id == search_product.id)
                            if move_search_product_code:
                                move_search_product_code.product_uom_qty += 1
                            else:
                                order_line_val = {
                            ...
                            move_search_product_code = self.move_ids_without_package.sudo().filtered(
                                lambda l: l.product_id.id == search_product_code.product_id.id)
                            if move_search_product_code:
                                move_search_product_code.product_uom_qty += 1
                            else:
                                order_line_val = {
                                    ...
                                    # "product_uom_qty": 1,
                                    "price_unit": 100,# search_product_code.lst_price,
                                    "location_id": self.location_id.id,
                                    "location_dest_id": self.location_dest_id.id,
                                    'sh_inventory_barcode_scanner_is_last_scanned': is_last_scanned,
                                    'sequence': sequence,
