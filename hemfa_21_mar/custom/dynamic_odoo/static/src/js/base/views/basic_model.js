odoo.define('dynamic_odoo.basic_model', function (require) {
    ...
    BasicModel.include({
        ...
        async _makeDefaultRecord(modelName, params) {
            const res = await this._super(modelName, params);
            if (params.type == "record" && params.viewType == "form") {
                await this.loadApproval(this.localData[res]);
            }
            return res;
        },
        // save: async function (recordID, options) {
        //     const res = await this._super(recordID, options);
        //     return res;
        // },
