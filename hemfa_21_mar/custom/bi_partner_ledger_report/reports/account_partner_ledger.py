...
class BiReportPartnerLedger(models.AbstractModel):
    ...
    def _lines(self, data, partner):
        ...
        domain = [('company_id', 'in', select_companies.ids)]
        if from_date:
    ...
    def _sum_partner(self, data, partner, field):
        ...
        params = [partner.id,tuple(data['computed']['move_state']), tuple(data['computed']['account_ids']),tuple(select_companies.ids)] + \
                 query_get_data[2]
        ...
        if not self.env.context.get('report_type') == "excel":
            new_domain = []
            new_domain += [('company_id', 'in', select_companies.ids)]

            if from_date:
                new_domain.append(('date','<',from_date))
        
            if self._context.get('used_context'):
                balance_query_get_data = self.env['account.move.line'].with_context(self._context.get('used_context'))._where_calc(domain).get_sql()
            else:
                balance_query_get_data = self.env['account.move.line']._where_calc(new_domain).get_sql()

            extra_balance_params = [partner.id,tuple(data['computed']['move_state']), tuple(data['computed']['account_ids']),tuple(select_companies.ids)] + \
                     balance_query_get_data[2]

            
            balance_sql = """SELECT sum(""" + field + """)
                    FROM """ + balance_query_get_data[0] + """, account_move AS m
                    WHERE "account_move_line".partner_id = %s
                        AND m.id = "account_move_line".move_id
                        AND m.state IN %s
                        AND account_id IN %s
                        AND m.company_id IN %s
                        AND """ + balance_query_get_data[1]

            self.env.cr.execute(balance_sql,extra_balance_params)
            initial_balance_row = self.env.cr.dictfetchall()

            if initial_balance_row :
                extra_balance = initial_balance_row[0].get('sum', 0.0) or 0.0
            else:
                extra_balance = 0.0

            if contemp is not None:
                result = contemp[0] + extra_balance
        else:
            if contemp is not None:
                result = contemp[0]
    ...
    def _calculate_initial_balancne(self,data,partner,final_row):
        ...
        if self._context.get('used_context'):
            ...
            query_get_data = self.env['account.move.line']._where_calc(domain).get_sql()
        params = [partner.id,date_from,tuple(data['computed']['move_state']),tuple(data['computed']['account_ids']),tuple(select_companies.ids)] 
    ...
    def _get_report_values(self, docids, data=None):
        ...
        abc = {
            ...
            'check_initial_balance': data.get('initial_balance')
        }
        return abc
