...
class ShCustomModeltemplate(models.Model):
    _name = 'sh.custom.tab.template'
    _description = 'Custom Tab template'
    ...
    def unlink(self):
        if self.view_id:
            self.view_id.unlink()
        res = super(ShCustomModeltemplate, self).unlink()
        return res

    def write(self, vals):
        inherit_id = self.env.ref('product.product_template_form_view')
        res = super(ShCustomModeltemplate, self).write(vals)
    ...
    def create_tab(self):
        ...
        ir_ui_view_obj = self.env['ir.ui.view'].create({'name': 'template.dynamic.tab',
                                                        'type': 'form',
                                                        'model': 'product.template',
                                                        'mode': 'extension',
                                                        'inherit_id': inherit_id.id,
