...
options.registry.s_weather_snippet = options.Class.extend({
    ...
    updateScript: function() {
        this.$target.find('.a_weather_snippet').attr('href', this._getselectDataAttribute());
        this.$target.find('.a_weather_snippet').data('label_1', this._getLabel());
        this.$target.find('.a_weather_snippet').data('theme', this._getTheme());
        this.$target.find('.a_weather_snippet').data('days', this._getDays());
        this.$target.find('.a_weather_snippet').data('mode', this._getMode());
        this.$target.find('.a_weather_snippet').data('icons', this._getIcons());
        this.$target.find('.a_weather_snippet').data('font', this._getFont());
        this.__weatherwidget_init(this.$target.find('.a_weather_snippet'));
    },
    ...
    __weatherwidget_init: function (a) {
        var a = a,
            i = [];
        if (0 !== a.length) {
    ...
    _computeWidgetState: function (methodName, params) {
        ...
        async _renderCustomXML(uiFragment) {
            this.cityLists = await rpc("/weather_info/", {
                model: 'weather.city',
