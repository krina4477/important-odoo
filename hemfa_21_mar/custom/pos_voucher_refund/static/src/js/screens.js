...
odoo.define("pos_voucher_refund.screens", function (require) {
    ...
    const PosResPaymentScreen = (PaymentScreen) => class extends PaymentScreen {
        ...
        async validateOrder(isForceValidate) {
            ...
            try {
                // Fetch default values asynchronously
                ...
                // Validate voucher refund conditions
                if (order.is_to_voucher_refund()) {
                    if (order.refund_voucher_amount < self.props.min_amount) {
                        await self.showPopup('MinAmountPopupWidget', {
                            'default_min_amount': self.props.min_amount,
                            'voucher_amount': order.refund_voucher_amount
                        });
                    } else if (order.refund_voucher_amount > self.props.max_amount) {
                        await self.showPopup('MaxAmountPopupWidget', {
                ...
                self.showPopup('ErrorPopupWidget', {
                    title: "Validation Error",
                    body: "An error occurred during validation. Please try again."
                });
            }
        }
        ...
        add_voucher_creation_product() {
            ...
            if(product){
                if (amount < 0) {
                    ...
                    order.add_product(product, { price: amount * -1 });
                }
            }
            else{
        ...
        click_voucher_refund(event) {
            ...
            console.log("================32=3=323=23=23=2=244432=32=3=32=3=32");
            if (order.is_to_voucher_refund()) {
