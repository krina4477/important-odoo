...
class StockPicking(models.Model):
	...
	def action_confirm(self):
		...
		if self.env.user.has_group('stock_force_date_app.group_stock_force_date'):
			for stock in self:
				stock.write({
						'scheduled_date' : self.force_date,
					})
		return res
	...
	def button_validate(self):
		...
		if self.env.user.has_group('stock_force_date_app.group_stock_force_date'):
			if res:
				for move in self:
					move_lines = self.env['stock.move.line'].search([('reference' , '=' , move.name)])
					if move_lines:
						move_lines.update({
							'date' : move.force_date,
						})
					move_ids = self.env['stock.move'].search([('move_line_ids' , 'in' , move_lines.ids)])
					if move_ids:
						move_ids.update({
							'date' : move.force_date,
						})
						stock_valuation_ids = self.env['stock.valuation.layer'].search([('stock_move_id' , 'in' , move_ids.ids)])
						if stock_valuation_ids:
							force_date = self.force_date
							self.env.cr.execute(
								"UPDATE stock_valuation_layer SET create_date = %s WHERE id in %s",
									[force_date, tuple(stock_valuation_ids.ids)])
		return res
