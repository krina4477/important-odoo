...
#    @api.depends('amount_total')
    @api.depends('analytic_account_id', 'amount_total')#13 Add project due to calculate invoice_to_date amount if project change
...
#            cus_inv = self.search([('state', 'in', ['open', 'paid']), ('partner_id', '=', rec.partner_id.id), ('project_id', '=', rec.project_id.id)])
            if rec.analytic_account_id:#odoo13
                cus_inv = self.search(
                    ['|', 
                        ('state', 'in', ['posted']),
                        ('payment_state', 'in', ['paid']),# ('invoice_payment_state', 'in', ['paid']),
...
class AccountMoveLine(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account')  # Temporary
