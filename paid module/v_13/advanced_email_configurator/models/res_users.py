...
class ResUser(models.Model):
    ...
    incoming_server_id = fields.Many2one(
        'fetchmail.server', string='Incoming User')
    provider_id = fields.Many2one('mail.server', string="Provider")
    is_required_field = fields.Boolean(
        string='Is Required', compute='_compute_required_field', default=_default_required_field)
    ...
    def test_connection(self):
        ...
        self._cr.commit()
        current_user = self.env['ir.mail_server'].search(
            [('user_id', '=', self.id)])
        ...
        current_incoming_user = self.env['fetchmail.server'].search(
            [('user_id', '=', self.id)])
        for server in current_user:
            smtp = False
            try:
                smtp = self.env['ir.mail_server'].connect(
                    user=self.email, password=self.password_preference, host=host_server, encryption=encryption_server, port=port_server)
    ...
    def set_mail_server(self):
        ...
        if current_incoming_user:
            ...
            incomingdict = {
                ...
                'user_id': self.id,
                'mail_server_id': self.provider_id.id,
                'server_type': self.provider_id.type
            }
            ...
            self.env['fetchmail.server'].sudo().create(incomingdict)
        return True
...
class ResPartner(models.Model):
    ...
    password_preference = fields.Char(
        string='Password', help="Optional password for SMTP authentication")
