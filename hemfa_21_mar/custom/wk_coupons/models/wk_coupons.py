...
class VoucherVoucher(models.Model):
    ...
    def get_default_values(self, field=False):
        ...
        vals['default_availability'] = irDefault.get('res.config.settings', 'wk_coupon_availability') or 1
    ...
    voucher_ids = fields.Many2many('voucher.voucher', 'voucher_template', 'voucher_id', 'parent_voucher_id',
                                   string='Vouchers')
    ...
    def create(self, vals):
        ...
        if vals['voucher_value'] < self.get_default_values('wk_coupon_min_amount') and vals[
        ...
        if not is_template and (
                vals.get('expiry_date') and datetime.now().date() > datetime.strptime(str(vals['expiry_date']),
                                                                                      '%Y-%m-%d').date()):
            raise ValidationError(('Expiry date have been passed already. Please select a correct expiry date..'))
        if not is_template and (
                vals.get('issue_date') and datetime.now().date() > datetime.strptime(str(vals.get('issue_date')),
                                                                                     '%Y-%m-%d').date()):
            raise ValidationError((
    ...
    def write(self, vals):
        for voucher in self:
            ...
            history_values = {}
            product_id = self.get_default_values('wk_coupon_product_id')
            voucher_value = vals.get('voucher_value') if vals.get('voucher_value') else self.voucher_value
            # voucher_value_type = vals.get('voucher_val_type') if vals.get('voucher_val_type') else self.voucher_val_type
            ...
            if voucher_value < self.get_default_values('wk_coupon_min_amount'):
                raise ValidationError(
                    ('You can`t create voucher below this minimum amount (%s) !!!') % self.get_default_values(
                        'wk_coupon_min_amount'))
            if vals.get('percnet'):
            ...
            if vals.get('voucher_value') and vals.get('voucher_value') > self.get_default_values(
                    'wk_coupon_max_amount'):
                raise ValidationError(
                    ('You can`t create a voucher greater than maximum amount (%s) !!!') % self.get_default_values(
                        'wk_coupon_max_amount'))
    ...
    def _validate_n_get_value(self, secret_code, wk_order_total, product_ids, refrence=False, partner_id=False):
        ...
        if not self_obj.is_partially_redemed and self_obj.voucher_value == 0:
            ...
            if self_obj.customer_type == 'general' and self_obj.voucher_value == 0:
                result['type'] = _('ERROR')
                result['message'] = _(
                    'Voucher value is 0. You can`t redeem this voucher anymore !!!')
                return result
    ...
    def generate_voucher_from_template(self, template_id):
        ...
        template = self.search(
            [('id', '=', int(template_id)), ('is_template', '=', True), ('active', '=', False)]).sudo()
        if template.exists():
            expiry_date = fields.Date.today() + fields.date_utils.relativedelta(
                days=(template.validity_in_days or 1) - 1)
