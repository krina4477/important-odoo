...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, qty, uom=None, date=False, **kwargs):
        self.ensure_one()
        if not date:
        ...
        if not uom and self._context.get('uom'):
            ...
            uom = products_qty_partner.uom_id.id    
        if uom:
            products = [item[0].with_context(uom=uom) for item in products_qty_partner]
            products_qty_partner = [(products_qty_partner,qty, self._context.get('partner_id'))]
        else:
        ...
        items = self.env['product.pricelist.item'].search([('id', 'in', item_ids), ('uom_id', '=', uom)])
        results = {}
        for product, qty, partner in products_qty_partner:
            ...
            for rule in items:
                ...
                if rule.base == 'pricelist' and rule.base_pricelist_id:
                    price_tmp = rule.base_pricelist_id._compute_price_rule(product, qty)[product.id][
