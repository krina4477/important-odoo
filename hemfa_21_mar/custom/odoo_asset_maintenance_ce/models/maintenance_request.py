...
#                    start_date = datetime.strptime(rec.custom_frequency_start_date, "%Y-%m-%d")
                    start_date = rec.custom_frequency_start_date
...
#                    'requisition_id': purchase_requisition.id,
            ...
            if lines:
                if not rec.custom_employee_id:
                    raise ValidationError("Employee Must be selected on Maintenance Request to create requisition")
                requisition_vals = {
...
#                if rec.maintenance_type == 'preventive':#Department is Mandatory field on requisition#V13
                if not rec.custom_department_id:
                    raise ValidationError("Department Must be selected on Maintenance Request to create requisition")
