...
from odoo.addons.prt_report_attachment_preview.controllers.report import CxReportController

class ReportControllerInherit(CxReportController):
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        ...
        if converter == 'pdf' and report.populating_ms_word_template:
            ...
            if report.type_export == 'pdf':
                ...
                docx = report.with_context(context).render_doc_doc(datas, data=data, report_ref=report.report_name)[0]
                docxhttpheaders = [
        ...
        try:
            if type in ['qweb-pdf', 'qweb-text']:
                ...
                if docids:
                    ...
                    # Particular report:
                    data_report = dict(url_decode(url.split('?')[1]).items())  # decoding the args represented in JSON
                    if 'context' in data_report:
                        context, data_context = json.loads(context or '{}'), json.loads(data_report.pop('context'))
                        context = json.dumps({**context, **data_context})
                    response = self.report_routes(reportname, converter=converter, context=context, **data_report)
