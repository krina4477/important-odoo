...
class KsWooCommerceConnectorInstance(models.Model):
    ...
    ks_invoice_tax_account = fields.Many2one('account.account', string="Invoice TAX Account", help="Show the tax account which will be used for invoice tax default account", domain="[('deprecated', '=', False), ('user_type_id.type', 'not in', ('receivable', 'payable')), ('company_id', '=', ks_company_id), ('is_off_balance', '=', False)]")
    ks_credit_tax_account = fields.Many2one('account.account', string="Credit Note TAX Account", help="Show the tax account which will be used for Credit Note/Refund tax default account", domain="[('deprecated', '=', False), ('user_type_id.type', 'not in', ('receivable', 'payable')), ('company_id', '=', ks_company_id), ('is_off_balance', '=', False)]")
    ...
    def ks_woo_connect_to_instance(self):
        ...
        try:
            ...
            if woo_api.get("").status_code == 200:
                ...
                self.ks_stock_field_type = self.env['ir.model.fields'].search([('name','=','free_qty')]).id
